...
export default function ScheduleCalendar({ currentStoreId, selectedTemplate, employees }: Props) {
  ...
  const startDate = startOfWeek(monthStart, { weekStartsOn: 1 }); // 월요일 시작
  const endDate = endOfWeek(monthEnd, { weekStartsOn: 1 }); // 월요일 기준의 끝(일요일)
    ...
    <div ref={calendarRef} style={{ backgroundColor: '#1a1a1a', padding: 20, borderRadius: 8, border: '1px solid #333', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {!isDeleteMode && <button onClick={handleDownloadImage} style={{ ...btnStyle, background: 'dodgerblue', border: 'none', fontWeight: 'bold' }}>📷 이미지 저장</button>}
          {isDeleteMode ? (
            <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', marginBottom: 10, textAlign: 'center' }}>
        {weeks.map((day, idx) => (
          // idx 5 = 토요일(파랑), idx 6 = 일요일(빨강)
          <div key={day} style={{ color: idx === 5 ? 'skyblue' : idx === 6 ? 'salmon' : '#aaa', fontWeight: 'bold', fontSize: 16 }}>{day}</div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderTop: '1px solid #444', borderLeft: '1px solid #444' }}>
        {calendarDays.map((day, idx) => {
          ...
          // 요일 인덱스 (0~6, 월~일 순서이므로 day.getDay()를 변환하거나 loop index 활용)
          // 간단하게: index % 7 === 5 (토), === 6 (일)
          const isSat = idx % 7 === 5;
          const isSun = idx % 7 === 6;
          const dayColor = isSun ? 'salmon' : isSat ? 'skyblue' : '#fff';
          ...
          return (
            <div 
              ...
              style={{ 
                minHeight: 130, padding: '4px 2px 20px 2px', borderRight: '1px solid #444', borderBottom: '1px solid #444',
                  ...
                  return (
                    <div 
                      ...
                      style={{
                        ...
                        color: '#fff', fontSize: 12, padding: '6px', borderRadius: 6, cursor: 'pointer', 
                        border: isDeleteMode ? (isSelectedForDelete ? '2px solid red' : '1px solid #555') : (sch.employee_id ? 'none' : '2px dashed #777'),
      ...
      {popupOpen && (
        <div style={{
          ...
          <div style={{ backgroundColor: '#222', padding: 24, borderRadius: 12, border: '1px solid #444', width: 360, boxShadow: '0 10px 25px rgba(0,0,0,0.8)' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <TimeSelector value={editStartTime} onChange={setEditStartTime} /> <span>~</span> <TimeSelector value={editEndTime} onChange={setEditEndTime} />
              </div>
            </div>
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 13, color: '#aaa', marginBottom: 8 }}>근무자 (대타)</label>
              <select 
                value={editEmpId || ''} onChange={(e) => setEditEmpId(e.target.value || null)}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              {!isNew ? <button onClick={handleDelete} style={{ padding: '10px 16px', background: '#c0392b', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 'bold' }}>삭제</button> : <div></div>}
              <div style={{ display: 'flex', gap: 8 }}>
