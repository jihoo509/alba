...
type Schedule = {
  ...
  is_holiday_work?: boolean; // ✅ [추가] 휴일근로 여부
};
...
export default function ScheduleCalendar({ currentStoreId, selectedTemplate, employees }: Props) {
  ...
  const handleScheduleClick = (e: React.MouseEvent, sch: Schedule) => {
    ...
    if (isDeleteMode) {
      ...
      setEditIsHolidayWork(sch.is_holiday_work || false); // 상태 세팅
      setIsNew(false);
      setPopupOpen(true);
    }
  };
  ...
  const handleDateClick = (day: Date) => {
    ...
    setEditIsHolidayWork(false); // 초기화
    setIsNew(true);
    setPopupOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      is_holiday_work: editIsHolidayWork, // ✅ 저장
      color: '#4ECDC4'
    };
